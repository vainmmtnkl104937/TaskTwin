...
`v1.0.0` production release. Severities follow `docs/severity.md`.
It is the live successor of `docs/uat/known-issues-v1.0.0-rc.1.md` and
carries the same inherited limitations forward unchanged.

## Status legend

- **Open** — defect not yet fixed in the current release.
- **Triaged** — root cause understood; fix tracked in a future session.
- **Closed** — fixed in the current release or accepted as a limitation
  outside V1 scope.

## Limitations inherited from prior sessions

These limitations were explicitly out of scope of the V1 browser-first
MVP and are not V1 release blockers. They are recorded for transparency.
...
## Defects found against v1.0.0

This section is empty at the time `v1.0.0` is promoted. Each defect
reported after promotion is recorded here with a severity and an owner.
...
This section is empty at the time `v1.0.0` is promoted. Promotion to the
next product release tag is blocked by any P0 entry here, and by any
acceptance-critical P1.
