    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        const owned = await tx.$queryRaw<Array<{ id: string }>>`
          SELECT "id" FROM "notification_outbox_messages"
        ...
        `;
        if (owned.length === 0) {
          return { outcome: 'permanent', safeErrorCode: 'OUTBOX_LEASE_LOST' };
        }
        const message = await tx.notificationOutboxMessage.findUnique({
          where: { id: messageId },
          include: { alert: true },
        });
        if (message === null)
          return { outcome: 'permanent', safeErrorCode: 'OUTBOX_LEASE_LOST' };
        if (
          !OperationalAlertTemplateSchema.safeParse(
            message.alert.templateParameters,
          ).success
        ) {
          return {
            outcome: 'permanent',
            safeErrorCode: 'ALERT_TEMPLATE_INVALID',
          };
        }
        if (
          !OperationalAlertActionTargetSchema.safeParse(
            message.alert.actionTarget,
          ).success
        ) {
          return {
            outcome: 'permanent',
            safeErrorCode: 'ALERT_ACTION_TARGET_INVALID',
          };
        }
        ...
        if (member === null)
          return {
            outcome: 'permanent',
            safeErrorCode: 'RECIPIENT_MEMBERSHIP_REVOKED',
          };
        ...
        if (message.alert.type.startsWith('run_')) {
          creatorEligible =
            (await tx.workflowRun.count({
              where: {
                id: message.alert.sourceId,
                workspaceId: message.workspaceId,
                createdByUserId: message.recipientUserId,
              },
            })) === 1;
        } else if (message.alert.type === 'schedule_auto_paused') {
          creatorEligible =
            (await tx.workflowSchedule.count({
              where: {
                id: message.alert.sourceId,
                workspaceId: message.workspaceId,
                createdByUserId: message.recipientUserId,
        ...
        if (!privileged && !creatorEligible) {
          return {
            outcome: 'permanent',
            safeErrorCode: 'RECIPIENT_NO_LONGER_ELIGIBLE',
          };
        }
        ...
        if (deliveredAt === undefined)
          return {
            outcome: 'retryable',
            safeErrorCode: 'DATABASE_TIME_UNAVAILABLE',
          };
      ...
      return {
        outcome: 'retryable',
        safeErrorCode: 'IN_APP_DELIVERY_TRANSIENT',
      };
    }
  }
}
