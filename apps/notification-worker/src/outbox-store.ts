...
export class NotificationOutboxStore {
  ...
  async retryOrDeadLetter(input: {
    ...
    const delay = input.retryable
      ? retryDelaySeconds(input.attemptCount)
      : null;
    ...
    return this.prisma.$transaction(async (tx) => {
      const owned = await tx.$queryRaw<Array<{ id: string }>>`
        ...
        FOR UPDATE
      `;
      if (owned.length === 0) return 'lost_lease';
      const locked = await tx.notificationOutboxMessage.findUnique({
      ...
      if (delay !== null) {
        const updated = await tx.$executeRaw`
          ...
          WHERE "id" = ${input.messageId}::uuid AND "status" = 'PROCESSING'
            ...
            AND "locked_until" > clock_timestamp()
        `;
        return updated === 1 ? 'retry_scheduled' : 'lost_lease';
      }
      ...
      await appendAuditEventTransactional(
        ...
        new WorkspaceAuditTrailRepository(tx),
        {
          ...
          payload: {
            alertId: locked.alertId,
            alertType: locked.alert.type,
            severity: locked.alert.severity,
            sourceType: locked.alert.sourceType,
            sourceId: locked.alert.sourceId,
            recipientCount: 1,
          },
        },
      );
