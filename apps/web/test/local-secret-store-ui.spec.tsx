...
describe('Local Secret Store Web metadata', () => {
  it('renders Runner status and aliases without secret values or reveal controls', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Local Secret Store' }),
    ).toBeInTheDocument();
    ...
    expect(screen.getByText('Installed Runner version')).toBeInTheDocument();
    expect(screen.getByText('windows / x64')).toBeInTheDocument();
    expect(screen.getByText('Compatible')).toBeInTheDocument();
    expect(screen.getByText('Runner protocol')).toBeInTheDocument();
    expect(screen.getByText('Workflow schema')).toBeInTheDocument();
    expect(screen.getByText('Local-state schema')).toBeInTheDocument();
    expect(document.body.textContent).not.toContain('WEB_SECRET_VALUE_29');
    expect(
      screen.queryByRole('button', { name: /reveal/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /install service/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /update|download|install/i }),
    ).not.toBeInTheDocument();
