...
const input = {
  ...
  template: {
    schemaVersion: 1 as const,
    templateKey: 'approval_required.v1' as const,
...
describe('OperationalAlertAppender', () => {
  const createMany = vi.fn(
    async (args: { data: unknown[]; skipDuplicates: boolean }) => {
      void args;
      return { count: 2 };
    },
  );
  ...
  const transaction = {
    ...
    operationalAlert: {
      upsert: vi.fn(async (args: { create: { contractDigest: string } }) => {
  ...
  it('routes only active OWNER and ADMIN and requests idempotent inserts', async () => {
    const result = await new OperationalAlertAppender().append(
      transaction,
      input,
    );
  ...
  it('rejects conflicting reuse of the stable source identity', async () => {
    digest = 'f'.repeat(64);
    await expect(
      new OperationalAlertAppender().append(transaction, input),
  ...
  it('can ignore a differently resolved legacy schedule alert during cleanup', async () => {
    const resolvedTransaction = {
      operationalAlert: {
        findUnique: vi.fn(async () => ({
          id: alertId,
          status: 'resolved',
          resolutionReason: 'resumed',
        })),
      },
    } as unknown as DatabaseTransactionClient;

    await expect(
      new OperationalAlertAppender().resolve(resolvedTransaction, {
        workspaceId,
    ...
    ).resolves.toEqual({ alertId, idempotent: true });
  });
});
